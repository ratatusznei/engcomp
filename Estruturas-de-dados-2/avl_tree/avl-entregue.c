...
int atualizar_altura (Arvore *a) {
	...
	int h_esq = a->esq == NULL? -1: atualizar_altura(a->esq);
	int h_dir = a->dir == NULL? -1: atualizar_altura(a->dir);

	return maior(h_esq, h_dir) + 1;
}
...
Arvore* atualizar_fb_dir (Arvore *a) {
	...
	if (balanceamento(a) == +2) {           // Pesada para a dir
		if (balanceamento(a->dir) >= 0) { // - Mesmo sinal ou zero
...
Arvore* atualizar_fb_esq (Arvore *a) {
	...
	if (balanceamento(a) == -2) {         // Pesada para a esq
		if (balanceamento(a->esq) <= 0) { // - Mesmo sinal ou zero
