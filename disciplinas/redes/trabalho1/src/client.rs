...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        println!("Usage: {} server:3400 filename.txt", args[0]);
        return Ok(());
    }
    ...
    if let Some(Message::Info {
        ...
        checksum,
        ..
    }) = get_info(&socket, server_ip, requested_file)
    {
        ...
        for byte in &file_data {
            calculated_checksum = calculated_checksum.overflowing_add(*byte as u32).0;
        }

        if checksum == calculated_checksum {
            fs::write("result.txt", &file_data)?;
            println!("Fire result.txt created.");
        } else {
            println!("Checksum recebido invalido, descartando arquivo...");
        }
    }
